export type LayoutProfile = 'desktop' | 'canvas';

declare global {
  interface Window {
    /** Set by the embedding host before game modules load; omitted in the desktop app. */
    __agentArcadeLayoutProfile?: LayoutProfile;
  }
}

const profile: LayoutProfile = window.__agentArcadeLayoutProfile === 'canvas' ? 'canvas' : 'desktop';
...
export function getAlienLayout(width: number, height: number) {
...
export function getGalaxyLayout(width: number, height: number) {
  const canvas = profile === 'canvas';
