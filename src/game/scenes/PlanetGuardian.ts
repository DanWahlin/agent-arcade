...
export class PlanetGuardianScene extends BaseScene {
  ...
  private renderPlayer(g: any) {
    ...
    if (this.shipSprite.texture.key !== texture) this.shipSprite.setTexture(texture);
  ...
  private renderSmartBombHUD() {
    if (this.displayedSmartBombs === this.smartBombs) return;
