...
export abstract class BaseScene extends Phaser.Scene {
  ...
  protected showGameOver(finalScore: number, restartFn: () => void) {
    ...
    modal.style.cssText = `
      ...
      text-align: center; min-width: 380px; max-width: 440px;
    ...
    board.forEach((s: number, i: number) => {
      ...
      row.style.cssText = `
        ...
        font-size: 12px; padding: 5px 12px; margin: 2px 0;
        border-radius: 6px;
      ...
      scoreEl.style.cssText = `
        ...
        font-size: 12px;
        ${isMe ? 'text-shadow: 0 0 6px rgba(255,235,59,0.4);' : ''}
      `;
    ...
    hint.style.cssText = 'color: #556; font-size: 9px; margin: 20px 0 0; letter-spacing: 1px;';
