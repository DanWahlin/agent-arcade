...
function createWindow() {
  ...
  win.webContents.on('before-input-event', (event, input) => {
    ...
    // Cmd+Q on Mac, Ctrl+Q on Windows/Linux
    if (input.key.toLowerCase() === 'q' && (isMac ? input.meta : input.control)) {
...
function createTray() {
  ...
  try {
    ...
    tray.setTitle('🍄');
    const isMac = process.platform === 'darwin';
    const toggleLabel = isMac ? 'Show / Hide  (⌃⌥M)' : 'Show / Hide  (Ctrl+Alt+M)';
    const quitLabel = isMac ? 'Quit  (⌘Q)' : 'Quit  (Ctrl+Q)';
    const menu = Menu.buildFromTemplate([
      { label: toggleLabel, click: toggleWindow },
      { type: 'separator' },
