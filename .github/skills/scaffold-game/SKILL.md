---
name: scaffold-game
description: Scaffold a new Agent Arcade mini-game. Use when adding a new game,
  creating a game scene, or scaffolding a game. Walks through every file that must
  be created or modified to add a fully integrated, tested game scene extending BaseScene.
---

# Scaffold a New Agent Arcade Game
...
export class <PascalName> extends BaseScene {
  ...
  /** One-line description shown on the ready screen. */
  protected getDescription(): string {
    return '<One-line gameplay description>';
  }

  /** Keyboard hints shown on the ready screen. */
  protected getControls(): { key: string; action: string }[] {
    return [
      // TODO: fill in the actual controls for this game
      { key: '← →', action: 'Move' },
      { key: 'SPACE', action: 'Shoot / Jump' },
      { key: 'P', action: 'Pause' },
    ];
  }
  ...
  preload() {
    ...
    // this.load.audio('my-sound', 'assets/<kebab-name>/sounds/my-sound.wav');
  }
  ...
  create() {
    ...
    // ── MUST be last — shows "press any key" overlay ──
...
- Call `this.startWithReadyScreen()` as the **last** line of `create()`.
- Do **NOT** call `this.input.keyboard.addCapture('SPACE')` before `startWithReadyScreen()` — it blocks the document keydown listener that dismisses the ready screen.
- Override `getDescription()` and `getControls()` — these are displayed on the ready screen.
- Use `this.addScore(pts, x, y)` for animated score bumps.
...
| `this.startWithReadyScreen()` | **Must** call last in `create()`. Shows "press any key" ready screen overlay. |
| `get displayName(): string` | Abstract getter — return the display name for the HUD and ready screen. |
| `getDescription(): string` | Override to return a one-line description shown on the ready screen. |
| `getControls(): { key, action }[]` | Override to return keyboard hints shown on the ready screen. |
...
| `this.spawnParticleExplosion(x, y, color, count)` | Spawn a tracked particle burst (auto-cleanup) |
| `this.activeEmitters` | Array of live particle emitters — cleaned up on shutdown |
| `this.destroyObj(obj)` | Safely destroy a Phaser game object and return `null` |
...
2. **Forgetting `this.startWithReadyScreen()`.** The game will start immediately with no ready screen. Must be the last call in `create()`.
3. **Forgetting `.js` extensions in imports.** TypeScript source uses `.js` extensions for ESM compatibility — this is intentional. Write `import { BaseScene } from './BaseScene.js'`, not `./BaseScene.ts` or `./BaseScene`.
4. **Hardcoded pixel positions.** Any value derived from screen size must use `W`/`H`. Tests will catch this at non-1080p viewports.
5. **Missing `declare const Phaser: any;`** at the top of the scene file. Phaser is loaded via `<script>` tag, not as an npm module.
6. **Score animation timing.** The HUD score has a 450 ms count-up animation. Tests must wait **≥ 500 ms** after calling `addScore()` before asserting the displayed score.
7. **Not adding the scene to the `GAMES` array.** The scene won't appear in the game-switcher dropdown.
8. **Key mismatch.** The `super('<kebab-name>')` key, `GAMES` array key, test `switchGame()` key, and asset folder name must all be the same `<kebab-name>` string.
9. **Forgetting `this.loadHighScore()`** in `create()` — the HUD will show 0 for the high score even if there's a saved value.
10. **Not resetting state in `create()`.** Scenes can be restarted — always reset `score`, `lives`, `level`, and game-specific state at the top of `create()`.
11. **Asset paths.** Use relative paths from the project root: `'assets/<kebab-name>/sprite.png'`, not absolute paths.
12. **Calling `addCapture('SPACE')` before the ready screen.** This blocks the document keydown listener that dismisses the ready screen overlay.
