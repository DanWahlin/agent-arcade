...
.github/workflows/ — CI/CD: ci.yml (PR checks), build.yml (releases), deploy-pages.yml (docs)
```
...
- **CI** (`ci.yml`): Runs on every PR and push to `main`. Performs TypeScript typecheck, builds frontend, and runs Playwright tests.
